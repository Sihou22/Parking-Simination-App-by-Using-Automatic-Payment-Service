#include "../../include/core/chars_segment.h"
...
Mat CCharsSegment::preprocessChar(Mat in) {//字符预处理
  // Remap image
  int h = in.rows;//获得图像的高度
  int w = in.cols;//获得图像的宽度
...
int CCharsSegment::charsSegment(Mat input, vector<Mat>& resultVec) {
  ...
  Color plateType = getPlateType(tmpMat, true);//获取车牌颜色

  Mat input_grey;
  cvtColor(input, input_grey, CV_BGR2GRAY);//转换为灰度图像
