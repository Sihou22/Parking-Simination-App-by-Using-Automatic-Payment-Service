...
//! 装载SVM模型
void CPlateDetect::loadSVM(const char* s) {
    PlateJudge::instance()->loadSVM(s);
}
...
int CPlateDetect::plateDetect(Mat src, std::vector<CPlate> &resultVec,
  ...
  m_plateLocate->plateColorLocate(src, color_Plates, index);//进行颜色查找
  PlateJudge::instance()->plateJudge(color_Plates, color_result_Plates);//进行车牌鉴定
  ...
  {
    m_plateLocate->plateSobelLocate(src, sobel_Plates, index);//进行Sobel定位
    PlateJudge::instance()->plateJudge(sobel_Plates, sobel_result_Plates);//进行车牌鉴定
  ...
  for (size_t i = 0; i < all_result_Plates.size(); i++) {
    ...
    resultVec.push_back(plate);//结果车牌
  }
  return 0;
}
